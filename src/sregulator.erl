...
update_meter(Now, open, #time{send=Send} = Time, Q, V,
    ...
    try VMod:open_time(V) of
        ...
        Class:Reason:Stacktrace ->
            valve_exception(Class, Reason, Stacktrace, open, Time, Q, V, Config)
    end;
update_meter(Now, closed, #time{send=Send, empty=Empty} = Time, Q, V,
    ...
    try QMod:send_time(Q) of
        ...
        Class:Reason:Stacktrace ->
            queue_exception(Class, Reason, Stacktrace, closed, Time, Q, V, Config)
    end.
...
closed({continue, From, Ref}, #time{send=Send, now=Now} = Time, Q, V, _,
    ...
    try VMod:handle_continue(Ref, Send, V) of
        ...
        Class:Reason:Stacktrace ->
            valve_exception(Class, Reason, Stacktrace, closed, Time, Q, V, Config)
    end;
...
common({update, From, Value}, State, #time{send=Send} = Time, Q, V, _,
    ...
    try VMod:handle_update(Value, Send, V) of
        ...
        Class:Reason:Stacktrace ->
            valve_exception(Class, Reason, Stacktrace, State, Time, Q, V, Config)
    end;
...
common({len, From, _}, State, Time, Q, V, _,
    ...
    try QMod:len(Q) of
        ...
        Class:Reason:Stacktrace ->
            queue_exception(Class, Reason, Stacktrace, State, Time, Q, V, Config)
    end;
...
opening_out(#time{now=Now, send=Send} = Time, Q, V, VNext,
    ...
    try QMod:handle_out(Now, Q) of
        ...
        Class:Reason:Stacktrace ->
            queue_exception(Class, Reason, Stacktrace, open, Time, Q, V, Config)
    end.
...
queue_exception(Class, Reason, Stacktrace, State, Time, Q, V,
                #config{queue_mod=QMod, valve_mod=VMod} = Config) ->
...
valve_exception(Class, Reason, Stacktrace, State, Time, Q, V,
                #config{queue_mod=QMod, valve_mod=VMod} = Config) ->
    Reason2 = {Class, Reason, Stacktrace},
    Callbacks = [{sbroker_queue, QMod, stop, Q},
